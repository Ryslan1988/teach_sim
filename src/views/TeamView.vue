...
<template>
  <div class="team-builder-screen">
    ...
    <section class="team-builder-layout">
      ...
      <aside class="team-recruit"><div class="recruit-head"><span>РЕКОМЕНДАЦИИ AI</span><b>ДОБРАТЬ 2</b></div><h2>Закройте пробелы</h2><p>Эти специалисты лучше всего дополняют выбранное ядро.</p><div class="recruit-list"><button v-for="c in suggestions" :key="c.id" class="recruit-card" :class="{ added: game.teamIds.includes(c.id) }" @click="game.addTeamMember(c.id)"><CandidateAvatar :candidate="c" size="sm"/><span><strong>{{ c.name }}</strong><small>{{ c.role }}</small></span><i>{{ game.teamIds.includes(c.id) ? '✓' : '+' }}</i></button></div></aside>
    </section>
    <footer class="team-footer"><span>Следующий шаг: определить техническую стратегию</span><button class="btn primary" :disabled="team.length < 5" @click="router.push('/project')">К ТЕХНИЧЕСКОМУ ЗАДАНИЮ&nbsp; →</button></footer>
  </div>
</template>
